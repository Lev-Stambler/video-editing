...
def convert_clip(path, output_path):
    ...
    command = "ffmpeg -i {} -ab 160k -ac 2 -ar 44100 -vn {} -y > /dev/null 2>&1".format(
        path, output_path)
...
def get_quiet_time(sound, loop_over_time):
    ...
    min_quiet_time = 1000
    turning_point = 10000  # audio must be above this level to be included
...
def create_new_clip(clip, quiet_opener, quiet_closer, speed_up_f):
    ...
    if abs(len(quiet_closer) - len(quiet_opener)) > 1:
        raise ValueError(
            "Opener and closer must be same or one diff in length")
    for i in range(0, len(quiet_closer)):
        ...
        if quiet_closer[i] > time_full_end or quiet_opener[i] > time_full_end:
            break
        sped_up = speed_change(
            clip[quiet_opener[i]:quiet_closer[i]], speed=speed_up_f)
        new_clip += sped_up
...
def create_audio_and_quiet_time(video_path, output_path, loop_over_time=100, speed_up_f=2):
    input_sound = "../files/audio_init.wav"
    ...
    print("Quiet markers", quiet_openers, quiet_closers)
